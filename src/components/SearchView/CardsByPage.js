...
const getDataURL = (kindURL, search, page, cinema_id, item_type, popularity_filters, ratings_filters, seasons_number, status) => {
  const base = config.base;
  const api = config.api;

  const parameters = getParameters(cinema_id, cinema_id_query, item_type, item_type_query, popularity_filters, popularity_filters_query, ratings_filters, ratings_filters_query, seasons_number, seasons_number_query, status, status_query);
...
const CardsByPage = ({ search, page, setPage, isLastPage, kindURL }) => {
  ...
  let { loading, data, error } = useFetch(getDataURL(kindURL, search, page, cinema_id, item_type, popularity_filters, ratings_filters, seasons_number, status_value));

  const [ref, inView] = useInView();

  const getDefaultItemType = (item_type_query, popularity_filters_query, seasons_number_query, status_query) => {
    if (item_type === "tvshow" || item_type_query === "tvshow" || typeof popularity_filters_query !== "undefined" || typeof seasons_number_query !== "undefined" || typeof status_query !== "undefined") return "tv";
